...
# Configuration:
my $playlist_file = defined(param('as')) ? "/root/pls/iptv-as.txt" : "/root/pls/iptv-nb.txt"; # TSV playlist (see m3u.pl)
...
use DBI;
my $dbh = DBI->connect("dbi:SQLite:dbname=${tvguide_file}","","", {RaiseError=>1, ReadOnly => 1}) or die("db disconnected");
...
while ( <$playlist> ) {
    ...
    if (!defined($category) || $category eq $p[2]) {
        ...
        say qq{<tr><td><a href="${caster}${p[1]}" target="_blank" class="caster"><img src="${p[3]}" /></a></td>};
        say qq{<td>${p[2]}</td>} if !defined($category);
        ...
        if($curpr->fetch) {
            say qq{<td>$percentage</td><td><div><div class="prog">$title</div><div class="desc"><tt>$desc</tt></div>$cat</div></td>};
        } else {
            say qq{<td></td><td></td>};
        }
        ...
        if($nextpr->fetch) {
            say qq{<td><div>(→${timetonext}) <div class="prog">$title</div><div class="desc"><tt>$desc</tt></div>$cat</div></td>};
        } else {
            say qq{<td></td>};
        }
        say qq{</tr>};
...
window.onload = function () {
    document.querySelectorAll(".prog").forEach(function(e){
        e.onclick = function() { toggle(e.nextSibling); }; 
        e.title = e.nextSibling.innerText;
    });
